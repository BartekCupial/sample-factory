...
class Learner(Configurable):
    ...
    def init(self) -> InitModelData:
        ...
        optimizer_cls = dict(
            ...
            adam_tensorflow_style=AdamTensorFlowStyle,
            lamb=Lamb,
        ...
        if self.cfg.optimizer in ["adam", "adam_tensorflow_style", "lamb"]:
    ...
    def _aux_kl_loss(self, old_action_logits, action_logits, valids, num_invalids: int) -> Tuple[Tensor, Tensor]:
    ...
    def _compute_model_outputs(self, mb: TensorDict, num_invalids: int, detach_critic: bool = False):
        ...
        with self.timing.add_time("tail"):
            ...
            result = self.actor_critic.forward_tail(
                core_outputs, values_only=False, sample_actions=False, detach_critic=detach_critic
            )
    ...
    def _aux_calculate_losses(self, mb: AttrDict, num_invalids: int) -> Tuple[ActionDistribution, Tensor, Tensor]:
        ...
        with self.timing.add_time("losses"):
            ...
            aux_value_loss = self._value_loss(values, old_values, targets, clip_value, valids, num_invalids)

        loss_summaries = dict(
            aux_values=result["values"],
        )

        return aux_kl_loss, aux_value_loss, loss_summaries
    ...
    def _aux_train(
        ...
        for epoch in range(self.cfg.aux_num_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with timing.add_time("losses_postprocess"):
                    loss: Tensor = aux_kl_loss + aux_value_loss
    ...
    def _aux_record_summaries(self, train_loop_vars) -> AttrDict:
        ...
        stats.aux_value_loss = var.aux_value_loss

...
from .roles import Alignment, Race, Role, Sex
...
def build_dataset_sql_query(
    ...
    subselect_sql += "version in ({seq}) AND ".format(seq=",".join(["?"] * len(game_versions)))
    subselect_sql_args = tuple(game_versions)

    # If specific game ids were specified
    if game_ids is not None:
        subselect_sql += "gameid in ({seq}) AND ".format(seq=",".join(["?"] * len(game_ids)))
        subselect_sql_args += tuple(game_ids)
    if roles is not None:
        subselect_sql += "role in ({seq}) AND ".format(seq=",".join(["?"] * len(roles)))
        subselect_sql_args += tuple(roles)
    if races is not None:
        subselect_sql += "race in ({seq}) AND ".format(seq=",".join(["?"] * len(races)))
        subselect_sql_args += tuple(races)
    if alignments is not None:
        subselect_sql += "align in ({seq}) AND ".format(seq=",".join(["?"] * len(alignments)))
        subselect_sql_args += tuple(alignments)
    if genders is not None:
        subselect_sql += "gender in ({seq}) AND ".format(seq=",".join(["?"] * len(genders)))
